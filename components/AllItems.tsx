...
type SortKey = 'name' | 'storeName' | 'category' | 'price' | 'date' | 'type';
...
const AllItems: React.FC<AllItemsProps> = ({ receipts, onUpdateReceipt }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Sorting logic
    result.sort((a, b) => {
      let aValue: any = a[sortConfig.key as keyof ItemWithMetadata];
      let bValue: any = b[sortConfig.key as keyof ItemWithMetadata];
  ...
  const SortIcon = ({ column }: { column: SortKey }) => {
    if (sortConfig.key !== column) return <svg className="w-3 h-3 ml-1 opacity-0 group-hover:opacity-40 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>;
    return (
      <svg className={`w-3 h-3 ml-1 text-blue-600 transition-transform ${sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th 
                  className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 cursor-pointer group hover:text-gray-900 transition-colors"
                  onClick={() => handleSort('name')}
                >
                  <div className="flex items-center">
                    Purchased Item <SortIcon column="name" />
                  </div>
                </th>
                <th 
                  className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 cursor-pointer group hover:text-gray-900 transition-colors"
                  onClick={() => handleSort('storeName')}
                >
                  <div className="flex items-center">
                    Store / Date <SortIcon column="storeName" />
                  </div>
                </th>
                <th 
                  className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 cursor-pointer group hover:text-gray-900 transition-colors"
                ...
                >
                  <div className="flex items-center">
                    Category / Sub <SortIcon column="category" />
                  </div>
                </th>
                <th 
                  className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 text-right cursor-pointer group hover:text-gray-900 transition-colors"
                  ...
                  <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col space-y-1">
                        {item.category && (
                          <span className="w-fit px-2 py-0.5 bg-gray-50 text-gray-600 text-[9px] font-black uppercase rounded-md border border-gray-100">
                            {item.category}
                          </span>
                        )}
                        {item.subcategory && (
                          <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest pl-1">
                            {item.subcategory}
                          </span>
                        )}
                      </div>
                    ...
                    <td className="px-6 py-5 text-right">
                      <span className={`text-sm font-black ${item.type === 'refund' ? 'text-emerald-600' : 'text-gray-900'}`}>
                        {item.type === 'refund' ? '-' : ''}${(item.price * item.quantity).toFixed(2)}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              <div className={`flex justify-between items-center p-6 rounded-[2rem] ${selectedReceipt.type === 'refund' ? 'bg-emerald-50/50' : 'bg-blue-50/50'}`}>
                <div>
                  <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${selectedReceipt.type === 'refund' ? 'text-emerald-400' : 'text-blue-400'}`}>
                    {selectedReceipt.type === 'refund' ? 'Amount Credited' : 'Total Bill'}
                  </p>
                  <p className={`text-3xl font-black ${selectedReceipt.type === 'refund' ? 'text-emerald-600' : 'text-blue-600'}`}>
                    {selectedReceipt.type === 'refund' ? '-' : ''}${selectedReceipt.total.toFixed(2)}
                  </p>
                </div>
