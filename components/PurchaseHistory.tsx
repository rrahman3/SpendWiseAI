...
const PurchaseHistory: React.FC<PurchaseHistoryProps> = ({ receipts, onScanClick, onUpdateReceipt }) => {
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  {receipt.type === 'refund' && (
                    <span className="text-[8px] font-black uppercase tracking-widest bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded border border-emerald-100">
                      Refund
                    </span>
                  )}
                </div>
              ...
              <span className={`text-lg font-bold ${receipt.type === 'refund' ? 'text-emerald-600' : 'text-gray-900'}`}>
                {receipt.type === 'refund' ? '-' : ''}${receipt.total.toFixed(2)}
              </span>
            </div>
            
            <div className="flex gap-2 mb-4">
              {Array.from(new Set(receipt.items.map(i => i.category))).slice(0, 2).map(cat => (
                <span key={cat} className="text-[10px] uppercase tracking-wider font-bold text-gray-600 bg-gray-50 px-2 py-0.5 rounded">
                  {cat || 'Item'}
                </span>
              ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in zoom-in-95">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className={`flex justify-between items-center p-4 rounded-2xl ${selectedReceipt.type === 'refund' ? 'bg-emerald-50/50' : 'bg-blue-50/50'}`}>
                <div>
                   <p className={`text-[10px] font-black uppercase ${selectedReceipt.type === 'refund' ? 'text-emerald-400' : 'text-blue-400'}`}>
                     {selectedReceipt.type === 'refund' ? 'Amount Credited' : 'Total Charged'}
                   </p>
                   <p className={`text-2xl font-black ${selectedReceipt.type === 'refund' ? 'text-emerald-600' : 'text-blue-600'}`}>
                    ...
                    <div key={idx} className="flex justify-between items-center p-3 border-b border-gray-100 last:border-0">
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-gray-500 font-bold uppercase">
                          {item.category}{item.subcategory ? ` • ${item.subcategory}` : ''} • Qty {item.quantity}
                        </p>
                      </div>
                      <p className={`text-sm font-black ${selectedReceipt.type === 'refund' ? 'text-emerald-600' : 'text-gray-900'}`}>
                        {selectedReceipt.type === 'refund' ? '-' : ''}${(item.price * item.quantity).toFixed(2)}
                      </p>
                    </div>
                  ))}
                </div>
