...
const ReceiptScanner: React.FC<ReceiptScannerProps> = ({ onReceiptProcessed, onFinished }) => {
  ...
  const processQueue = async () => {
    ...
    try {
      ...
      // Fix: Added missing 'type' property required by Receipt interface
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {queue.map((item) => (
                  <div 
                    ...
                    className={`relative group aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm transition-all bg-gray-50 cursor-pointer ${
                      item.status === 'verified' ? 'hover:ring-8 ring-blue-500/10' : ''
                    }`}
                  >
                    <img src={item.preview} alt="Preview" className="w-full h-full object-cover" />
                    
                    <div className={`absolute inset-0 flex flex-col items-center justify-center px-4 text-center transition-all ${
                      item.status === 'processing' ? 'bg-blue-600/70 backdrop-blur-md' : 
            ...
            <div className="pt-10 border-t border-gray-100 flex flex-col md:flex-row gap-6 items-center justify-between">
              ...
              <div className="flex space-x-4 w-full md:w-auto">
                ...
                <button 
                   onClick={onFinished} 
                   className={`flex-1 md:flex-none px-14 py-5 font-black rounded-[1.5rem] transition-all shadow-2xl ${
                     allDone ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200' : 'bg-gray-100 text-gray-300 cursor-not-allowed shadow-none'
                   }`}
                   disabled={!allDone}
                >
