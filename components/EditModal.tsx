...
const EditModal: React.FC<EditModalProps> = ({ receipt, onSave, onClose }) => {
  ...
  const addItem = () => {
    setFormData({ 
      ...formData, 
      items: [...formData.items, { name: 'New Item', price: 0, quantity: 1, category: 'Other', subcategory: '' }] 
    });
  };
            ...
            <div className="max-w-xl mx-auto space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex p-1 bg-gray-100 rounded-[1.5rem]">
                    <button 
                      onClick={() => setFormData({ ...formData, type: 'purchase' })}
                      className={`flex-1 py-3 text-xs font-black uppercase rounded-2xl transition-all ${
                        formData.type === 'purchase' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'
                      }`}
                    >
                      Purchase
                    </button>
                    <button 
                      ...
                      className={`flex-1 py-3 text-xs font-black uppercase rounded-2xl transition-all ${
                        formData.type === 'refund' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-400'
                      }`}
                    >
                      Refund
                    </button>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-5">
                  {formData.items.map((item, idx) => (
                    <div key={idx} className="p-6 bg-gray-50 rounded-[2rem] flex flex-col gap-4 group relative border-2 border-transparent hover:border-blue-100 transition-all shadow-sm">
                      <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => removeItem(idx)} className="p-2 text-gray-300 hover:text-red-500 transition-all hover:bg-red-50 rounded-xl">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          <label className="text-[9px] font-black uppercase text-gray-400 pl-1">Category</label>
                          <input 
                            className="w-full bg-white px-4 py-2 rounded-xl border border-gray-100 text-xs font-bold text-blue-600 outline-none"
                            value={item.category}
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] font-black uppercase text-gray-400 pl-1">Subcategory</label>
                          <input 
                            className="w-full bg-white px-4 py-2 rounded-xl border border-gray-100 text-xs font-bold text-gray-600 outline-none"
                      ...
                      <div className="flex items-center space-x-4 pt-2">
                        <div className="flex-1 flex items-center bg-white px-4 py-3 rounded-2xl border border-gray-100 shadow-sm">
                           ...
                           <input 
                              type="number" 
                              className="w-full bg-transparent text-sm font-black text-gray-900 outline-none"
                              value={item.price}
                              onChange={(e) => updateItem(idx, 'price', parseFloat(e.target.value) || 0)}
                           />
                        </div>
                        <div className="w-24 flex items-center bg-white px-4 py-3 rounded-2xl border border-gray-100 shadow-sm">
                           <span className="text-xs text-gray-300 font-black mr-2">×</span>
                           <input 
                              type="number" 
                              className="w-full bg-transparent text-sm font-black text-gray-900 outline-none text-center"
                              value={item.quantity}
              ...
              <div className="pt-10 border-t border-gray-100 flex justify-between items-center">
                <div className="space-y-1">
                   <p className="text-[11px] font-black uppercase text-gray-400 tracking-[0.2em]">Record Total</p>
                   <p className={`text-5xl font-black tracking-tighter ${formData.type === 'refund' ? 'text-emerald-600' : 'text-blue-600'}`}>
                     {formData.type === 'refund' ? '-' : ''}${formData.items.reduce((sum, i) => sum + (i.price * i.quantity), 0).toFixed(2)}
                   </p>
                </div>
                <button 
                  onClick={() => {
